...
graph_t graph_empty(unsigned int max_vertexs)
{
     ...
     * TODO: COMPLETAR.
     */
    graph->costs = calloc(graph->max_vertexs, sizeof(*(graph->costs)));
    for(unsigned int i=0u; i < graph->max_vertexs; i++){
        graph->costs[i] = calloc(graph->max_vertexs, sizeof(*(graph->costs[i])));    
    }
